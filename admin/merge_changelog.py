...
def _split_changelog(changelog):
...
def merge_changelogs(changelogs):
    preamble, main_changes = split_changelog(changelogs[0])
    for changelog in changelogs[1:]:
        dummy, changes = split_changelog(changelog)
        main_changes.extend(changes)
    main_changes.sort(key=lambda x: x[0], reverse=True)
    # Now remove duplicates (retain the first one)
    new_changes = [main_changes[0][1]]
    for left, right in itertools.pairwise(main_changes):
        if left[0] != right[0]:
            new_changes.append(right[1])
        elif left[1] != right[1]:
            print(f"Warning: Changes for version {left[0]} differ.")
    return preamble + "".join(new_changes)
...
def main():
    ...
    changelogs = [main_changelog]
    for branch in branches:
        print(f"Looking at branch {branch}")
        try:
            changelogs.append(get_changelog(repo, branch))
        except GitCommandError:
            print("No changelog found on this branch.")
            continue
    new_changelog = merge_changelogs(changelogs)

    if new_changelog != main_changelog:
        print("New versions have been added.")
